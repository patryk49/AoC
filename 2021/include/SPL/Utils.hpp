...
template<class T> static
void init(Span<T> range) noexcept{
	for (T *I=range.ptr; I!=range.ptr+range.size; ++I) init(*I);
}

template<class T> static
void deinit(Span<T> range) noexcept{
...
template<class T> static constexpr bool needs_init<Array<T>> = needs_init<T>;
template<class T> static constexpr bool needs_deinit<Array<T>> = needs_deinit<T>;
...
template<class T, class A> static
bool expand_back(DynamicArray<T, A> &arr, size_t amount, A &allocator) noexcept{
	size_t size = arr.size + amount;
	size_t bytes = size * sizeof(T);
	if (arr.data.size < bytes){
		Memblock blk;
		blk = realloc(allocator, arr.data, bytes);
		if (blk.ptr == nullptr) return true;
		arr.data = blk;
	}
...
bool push_value(Array<T> &arr, const TV &value, A &allocaotor) noexcept{
...
bool push_range(Array<T> &arr, Span<TV> range, A &allocator) noexcept{
	...
	if (arr.data.size < size*sizeof(T)){
		Memblock blk;
		blk = realloc(allocator, arr.data, size);
		if (blk.ptr == nullptr) return true;
		arr.data = blk;	
	}
...
template<class T> static
T &&pop_value(Array<T> &arr) noexcept{ return (T &&)((T *)arr.data.ptr)[--arr.size]; }
