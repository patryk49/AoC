...
template<class M>
SP_CSI size_t rows(const MatrixExprCopy<M> &m) noexcept{
	return rows(m.arg.rows);
}

template<class M>
SP_CSI size_t cols(const MatrixExprCopy<M> &m) noexcept{
	return cols(m.arg.cols);
}

template<class M>
SP_CSI size_t len(const MatrixExprCopy<M> &m) noexcept{
	return len(m.arg.len);
}

template<class M>
SP_CSI size_t cap(const MatrixExprCopy<M> &m) noexcept{
	return len(m.arg);
};
...
struct MatrixExprDynGenerator{
	...
	template<class O = Operation> SP_CI
	std::enable_if_t<std::is_invocable_v<O, size_t, size_t>, ValueType>
	operator ()(size_t r, size_t c) noexcept{ return operation(r, c); }
	
	template<class O = Operation> SP_CI
	std::enable_if_t<std::is_invocable_v<O>, ValueType>
	operator ()(size_t r, size_t c) noexcept{ return operation(); }
};
...
auto cp(const M &arg) noexcept{
	return MatrixExprCopy<typename std::decay_t<M>::ValueType>{arg};
}
...
template<class Operation>
auto generate(
	uint32_t rows, uint32_t cols, const Operation &operation
) noexcept{
	return MatrixExprDynGenerator<const Operation &>{
		rows, cols, operation
	};
}
...
auto generate(
	...
	return MatrixExprDynGenerator<Operation &>{
		rows, cols, operation
	};
}
...
auto generate(
	...
	return MatrixExprDynGenerator<typename std::decay_t<Operation>>{
		rows, cols, operation
	};
}
...
struct VectorExprDynGenerator{
	...
	template<class O = Operation> constexpr
	std::enable_if_t<std::is_invocable_v<O, size_t>, ValueType>
	operator [](size_t i) noexcept{ return operation(i); }
	
	template<class O = Operation> constexpr
	std::enable_if_t<std::is_invocable_v<O>, ValueType>
	operator [](size_t i) noexcept{ return operation(); }
};
...
auto generate(uint32_t size, Operation &operation) noexcept{
	return VectorExprDynGenerator<Operation &>{
		operation, size
	};
}

template<class Operation>
auto generate(uint32_t size, const Operation &operation) noexcept{
	return VectorExprDynGenerator<const Operation &>{
		operation, size
	};
}
...
template<class Operation>
auto generate(uint32_t size, Operation &&operation) noexcept{
	return VectorExprDynGenerator<typename std::decay_t<Operation>>{
		operation, size
	};
}
