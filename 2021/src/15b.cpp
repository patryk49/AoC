...
int main(int argc, char **argv){
	...
	constexpr size_t tilesWidth = 5;
	size_t newRows = rows * tilesWidth;
	size_t newCols = cols * tilesWidth;

	sp::DArray<uint8_t> newRisks(newRows*newCols);
	
	for (size_t i=0; i!=tilesWidth; ++i){
		for (size_t j=0; j<=i; ++j)
			for (size_t r=0; r!=rows; ++r)
				for (size_t c=0; c!=cols; ++c)
					newRisks[(r+(i-j)*rows)*newCols+(c+j*cols)] = risks[r*cols+c];
		for (auto &I : risks)
			I += 1 - (9 & -(I==9));
	}
	for (size_t i=1; i!=tilesWidth; ++i){
		for (size_t j=i; j!=tilesWidth; ++j)
			for (size_t r=0; r!=rows; ++r)
				for (size_t c=0; c!=cols; ++c)
					newRisks[(r+(tilesWidth+i-j-1)*rows)*newCols+(c+j*cols)] = risks[r*cols+c];
		for (auto &I : risks)
			I += 1 - (9 & -(I==9));
	}
	...
	for (size_t i=0; i!=newRows; ++i){
		for (size_t j=0; j!=newCols; ++j) putchar('0'+newRisks[i*newCols+j]);
		putchar('\n');
	}



	printf("\nshortest path: %u\n", findPath(newRisks, newRows, newCols));
