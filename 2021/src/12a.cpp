...
size_t getWays(sp::DArray<uint8_t [2]> &adjList, const uint8_t index = 0) noexcept{
	const bool isLittle = index < (1<<7);

	sp::DArray<uint8_t [2]> tempList;
	if (isLittle) moveSelected(tempList, adjList, index);
	else copySelected(tempList, adjList, index);

	size_t result = 0;
	for (auto I=std::begin(tempList); I!=std::end(tempList); ++I){
		const uint8_t next = (*I)[(*I)[0] == index];
		result += next==1 ? 1 : getWays(adjList, next);
	}

	if (isLittle){
		const size_t size = std::size(adjList);
		adjList.expandBy(std::size(tempList));
		memcpy(
			std::begin(adjList)+size, std::begin(tempList), std::size(tempList)*sizeof(tempList[0])
		);
	}
	return result;
}
