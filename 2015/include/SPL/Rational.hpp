...
template<class Base>
struct Rational{
	constexpr Rational() noexcept : nom{}, den{Base{1}}{}
	constexpr Rational(const Base nominator, const Base denominator) noexcept : nom{nominator}, den{denominator}{}
	...
	constexpr void simplify() noexcept{
		const Base divider = std::gcd(nom, den);
		if (divider != 1){
			nom /= divider;
			den /= divider;
		}
